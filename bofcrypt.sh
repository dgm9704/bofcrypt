...
SESSIONKEY=`openssl rand -hex 64`
...
<ENCRYPTED_REPORT xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  ...
  <VERSION>2.0</VERSION>
  <SESSION_KEY>`echo $SESSIONKEY | openssl rsautl -encrypt -inkey $PUBLICKEY -pubin -out >(base64)`</SESSION_KEY>
  <OUT_BUFFER>`openssl enc -aes-256-cbc -pbkdf2 -a -pass pass:$SESSIONKEY -in $INPUTFILE`</OUT_BUFFER>
</ENCRYPTED_REPORT>
EOF
